...
КРИТИЧЕСКИ ВАЖНО: Если пользователь просит создать лендинг/сайт/страницу — ОБЯЗАТЕЛЬНО вызови createLandingPage("тема"). НЕ пиши текст в ответ, ВЫЗОВИ createLandingPage. Это самый важный приоритет. Не используй saveFile, downloadFile, webSearch, readFile для лендингов — createLandingPage сам всё сделает.
...
- generateImage — генерация изображений (авто-сохранение в workspace). Используй ТОЛЬКО когда пользователь ЯВНО попросил картинку/изображение/иллюстрацию.
- runCode — выполнение JavaScript-кода (анализ данных, сортировка, генерация отчётов, таблиц)
- saveFile — сохранение файла в рабочее пространство. НЕ ИСПОЛЬЗУЙ для лендингов — есть createLandingPage.
...
- createLandingPage — СОЗДАТЬ КРАСИВЫЙ ЛЕНДИНГ НА ЛЮБУЮ ТЕМУ. Генерирует текст, ищет картинки, встраивает их, сохраняет HTML. Всё сам, ничего дополнительно вызывать не нужно. Один вызов — готовый лендинг.
- generatePDF — создать PDF из Markdown-текста. Используй ТОЛЬКО когда пользователь ЯВНО попросил PDF. НЕ добавляй изображения если пользователь их не просил. Если пользователь просит изображения — сначала generateImage, получи реальный URL /api/workspace/chatId/img-NNN.png, потом вставь в content как ![описание](/api/workspace/chatId/img-NNN.png). НЕ ВЫДУМЫВАЙ URL картинок.
...
- analyzeVideo — Единый инструмент для видео: извлекает инфо, ключевые кадры (base64 PNG), аудио и транскрипцию за ОДИН вызов. Возвращает готовую сводку (инфо + 5 кадров как base64 + транскрипция) для анализа vision API в одном шаге. Предпочитай его вместо ручной цепочки getVideoInfo→extractVideoFrames→extractVideoAudio→transcribeAudioFile
- installSkillFromGitHub — Поиск и установка навыков (скиллов) для агента из GitHub. С query="тема" ищет файлы навыков. Когда пользователь выбирает — вызывай снова с installUrl="ссылка" для установки. Автоматически проверяет промпт на опасные инструкции.
      ...
      for (const part of msg.parts) {
        if (part.type === "text" && (part as { text: string }).text) {
          ...
          if (mediaType.startsWith("image/")) {
            ...
            content.push({
              type: "text",
              text: `[Загружен файл: ${filename}]\n(используй analyzeVideo для полного анализа за один шаг)`,
            });
          } else {
...
export async function streamChatResponse(
  ...
  const hasImage = coreMessages.some(
    (m) => m.role === "user" && Array.isArray(m.content) && (m.content as Array<{ type: string }>).some((p) => p.type === "image")
  );
  const visionModel = "openai/gpt-4o";
  const finalModel = hasImage && !effectiveModel.toLowerCase().includes("vision") && !effectiveModel.toLowerCase().includes("gpt-4o")
    ? visionModel
    : effectiveModel;
  const activeModel = finalModel
    ? `${finalModel}@provider=${POLZAAI_PROVIDER}&allow_fallbacks=false`
    : POLZAAI_MODEL;
  console.log(`[polza-client] calling streamText with model=${activeModel}${effectiveTemperature != null ? `, temperature=${effectiveTemperature}` : ""}${hasImage ? " (vision auto-switch)" : ""}`);
...
/**
 * Обработать текстовое сообщение и получить финальный ответ (без SSE).
 * Используется для Telegram-polling и других не-браузерных сценариев.
 */
export async function getTextResponse(
  ...
  for (const cfg of settings.mcpServers) {
    if (cfg.enabled) {
      try { await connectMcpServer(cfg); } catch { /* ignore */ }
    }
  }
  const mcpAiTools = buildMcpAiTools();
  const allTools = { ...tools, ...mcpAiTools };

  const effectiveModel = settings.model || POLZAAI_MODEL_RAW;
  const effectiveTemperature = settings.temperature ?? undefined;
  ...
  const coreMessages = await uiMessagesToCoreMessages([userMsg], chatId);
  ...
  if (facts.length > 0) {
    ...
    coreMessages.push({
      role: "system",
      content: `[Известные факты о пользователе]\n${factsText}`,
    });
  }
  ...
  const memories = await getRecentMemories(5);
  if (memories.length > 0) {
    const memoriesText = memories.map((m) => `- [${m.title}] ${m.summary}`).join("\n");
    coreMessages.push({
      role: "system",
      content: `[Недавние диалоги]\n${memoriesText}`,
    });
  }

  const activeModel = `${effectiveModel}@provider=${POLZAAI_PROVIDER}&allow_fallbacks=false`;
  ...
  try {
    ...
    setTimeout(async () => {
      try {
        const extractRes = await fetch(`${POLZAAI_BASE_URL}/chat/completions`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.POLZAAI_API_KEY}` },
          body: JSON.stringify({
            model: EXTRACT_MODEL,
            messages: [{ role: "user", content: `Извлеки новые факты о пользователе. Верни JSON-массив: [{"text":"...","category":"personal|work|preference|hobby|goal|other"}]. Если нет — [].\n\nПользователь: ${text}\n\nАссистент: ${fullText}` }],
    ...
    setTimeout(async () => {
      try {
        ...
        await saveSessionMemory(chatId, text.slice(0, 80), summary);
      } catch { /* silent */ }
    }, 0);

    return fullText;
  } catch (err) {
    console.error(`[getTextResponse] error:`, err);
